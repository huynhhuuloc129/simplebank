...
migrateup1:
	migrate -path db/migration -database "postgresql://root:Codnumber1postgres@localhost:5432/simple_bank?sslmode=disable" -verbose up 1
...
migratedown1:
	migrate -path db/migration -database "postgresql://root:Codnumber1postgres@localhost:5432/simple_bank?sslmode=disable" -verbose down 1
...
mock:
	mockgen -package mockdb -destination db/mock/store.go github.com/huynhhuuloc129/simplebank/db/sqlc Store
